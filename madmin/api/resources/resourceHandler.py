...
class ResourceHandler(apiHandler.APIHandler):
    ...
    def create_routes(self):
        ...
        if self.component:
            route = self.uri_base
            self._app.route(route, methods=['GET', 'POST'], endpoint='api_%s' % (self.component,))(self.entrypoint)
            if self.iterable:
                route = '%s/<string:identifier>' % (self.uri_base,)
                methods = ['DELETE', 'GET', 'PATCH', 'PUT']
                if self.has_rpc_calls:
                    methods.append('POST')
                self._app.route(route, methods=methods, endpoint='api_%s' % (self.component,))(self.entrypoint)
    ...
    def process_request(self, *args, **kwargs):
        ...
        try:
            ...
            return (error, 400)
        except (utils.data_manager.ModeNotSpecified, resource_exceptions.NoModeSpecified):
