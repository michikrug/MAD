        ...
        try:
            ...
            for stop, error_count in self._stops_not_processed.items():
                if stop not in self._stoplist:
                    ...
                    self.logger.warning("Found stop not processed yet: {}".format(str(stop)))
    ...
    def _start_routemanager(self):
        ...
        try:
            if not self._is_started:
                ...
                if len(stops) == 0:
                    self.logger.info('No unprocessed Stops detected in route {} - quit worker', str(self.name))
                    self._shutdown_route = True
                    self._restore_original_route()
                ...
                if 0 < len(stops) < len(self._route) \
                    ...
                    self.logger.warning(
                        "Something wrong with area {}: it have many new stops - better recalc route!",
                        str(self.name))
