...
class MITMReceiver(object):
    ...
    def received_data_worker(self):
        while True:
            ...
            log.debug(
                "MITM data processing worker retrieved data. Queue length left afterwards: %s" % str(items_left))
            if items_left > 50:  # TODO: no magic number
                log.warning(
                    "MITM data processing workers are falling behind! Queue length: %s" % str(items_left))
            if item is None:
    ...
    def process_data(self, received_timestamp, data, origin):
        ...
        if origin not in self.__mitm_mapper.playerstats:
            log.warning(
                "Not processing data of %s since origin is unknown" % str(origin))
            return
        type = data.get("type", None)
        if type:
            try:
