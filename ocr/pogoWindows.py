...
class PogoWindows:
    ...
    def __readCircleCount(self, filename, hash, ratio, communicator, xcord=False, crop=False, click=False, canny=False,
        ...
        logger.debug("__readCircleCount: Determined screenshot scale: " +
        ...
        logger.debug("__readCircleCount: Detect radius of circle: Min " +
        ...
        if circles is not None:
            ...
            for (x, y, r) in circles:
                ...
                if not xcord:
                    ...
                    if click:
                        logger.debug(
                            '__readCircleCount: found Circle - click it')
                        communicator.click(
            ...
            logger.debug(
                "__readCircleCount: Determined screenshot to have 0 Circle")
            return -1
    ...
    def __checkRaidLine(self, filename, hash, communicator, leftSide=False, clickinvers=False):
        ...
        logger.debug("__checkRaidLine: Determined screenshot scale: " +
                     str(height) + " x " + str(width))
    ...
    def __checkClosePresent(self, filename, hash, communicator,  radiusratio=12, Xcord=True):
        if not os.path.isfile(filename):
            logger.warning(
                "__checkClosePresent: {} does not exist", str(filename))
            return False
    ...
    def checkCloseExceptNearbyButton(self, filename, hash, communicator, closeraid=False):
        logger.debug(
            "checkCloseExceptNearbyButton: Checking close except nearby with: file {}, hash {}", filename, hash)
        try:
            screenshotRead = cv2.imread(filename)
        except:
            logger.error("Screenshot corrupted :(")
            logger.debug(
                "checkCloseExceptNearbyButton: Screenshot corrupted...")
            return False
        if screenshotRead is None:
            logger.error(
                "checkCloseExceptNearbyButton: Screenshot corrupted :(")
        ...
        if not closeraid:
            logger.debug(
                "checkCloseExceptNearbyButton: Raid is not to be closed...")
            if (not os.path.isfile(filename)
    ...
    def checkpogomainscreen(self, filename, hash):
        logger.debug(
            "checkpogomainscreen: Checking close except nearby with: file {}, hash {}", filename, hash)
        mainscreen = 0
        try:
            screenshotRead = cv2.imread(filename)
        except Exception:
    ...
    def checkCloseButton(self, filename, hash, communicator):
        logger.debug(
            "checkCloseButton: Checking close with: file {}, hash {}", filename, hash)
        try:
            screenshotRead = cv2.imread(filename)
        except Exception:
            logger.error("Screenshot corrupted :(")
