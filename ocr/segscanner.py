...
class Scanner:
    def __init__(self, args, db_wrapper, hash):
        ...
        if not os.path.exists(self.unknownPath):
            logger.info('Unknow directory created')
            os.makedirs(self.unknownPath)

        if not os.path.exists(self.www_hash):
            logger.info('www_hash directory created')
    ...
    def detectRaidTime(self, raidpic, hash, raidNo, radius):
        ...
        logger.debug('[Crop: {} ({})] detectRaidTime: detect raidtimer text: {}', str(
            ...
            if ':' in raidtimer:
                ...
                if hatchTime:
                    logger.info('[Crop: {} ({})] ' + 'detectRaidTime: Hatchtime {}',
                                str(raidNo), str(self.uniqueHash), str(hatchTime))
                    # if hatchTime > unixnow + (int(args.raid_time) * 60) or hatchTime < unixnow:
                    # logger.info('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) +') ] ' + 'detectRaidTime: Hatchtime not logical')
                    ...
                    logger.debug('[Crop: {} ({})] detectRaidTime: Start: {} End: {}', str(
                        raidNo), str(self.uniqueHash), str(raidstart), str(raidend))
                    return (raidFound, True, raidstart, raidend)
                else:
                    logger.info(
                        '[Crop: {} ({})] ' + 'detectRaidTime: Could not read Raidtime', str(raidNo), str(self.uniqueHash))
                    return (raidFound, True, False, False)

            else:
                logger.info('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' +
    ...
    def detectRaidEndtimer(self, raidpic, hash, raidNo, radius):
        ...
        height, width, channel = raidpic.shape
        raidtimer = raidpic[int(round(radius * 2 * 0.03) + (2 * radius) + (radius * 2 * 0.10)):int(
            round(radius * 2 * 0.03) + (2 * radius) + (radius * 2 * 0.23)), 0:width]
        ...
        if raidEndFound:
            if ':' in raidtimer:
                logger.info('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' +
                            'detectRaidEndtimer: found raidendtimer {}', raidtimer)
                endTime = self.getEndTime(raidtimer, raidNo)
                if endTime:
                    logger.info('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) +
                                ') ] ' + 'detectRaidEndtimer: Endtime {}', str(endTime))
                    ...
                    # raidend = getHatchTime(self, raidtimer) + int(45*60) - (self.timezone*60*60)
                    logger.info('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) +
                                ') ] ' + 'detectRaidEndtimer: End: {}', str(raidend))
    ...
    def detectRaidBoss(self, raidpic, lvl, hash, raidNo):
        ...
        if monHash is None:
            for file in glob.glob("ocr/mon_img/_mon_*_" + str(lvl) + ".png"):
                logger.debug("Comparing to {}", str(file))
                find_mon = mon_image_matching(
                    self.args, file, picName, raidNo, hash)

                if not foundmon or find_mon > foundmon[0]:
    ...
    def detectLevel(self, raidpic, hash, raidNo, radius):
        ...
        height, width, channel = raidpic.shape
        raidlevel = raidpic[int(round(radius * 2 * 0.03) + (2 * radius) + (radius * 2 * 0.43)):int(
            round(radius * 2 * 0.03) + (2 * radius) + (radius * 2 * 0.68)), 0:width]
        ...
        if lvl >= 1 and lvl <= 5:
            ...
            logger.info('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) +
                        ') ] ' + 'detectLevel: found level {}', str(lvl))
            return lvl
    ...
    def detectGym(self, raidpic, hash, raidNo, captureLat, captureLng, radius, monId=None):
        ...
        if monId:
            logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) +
                         ') ] ' + 'detectGym: Got Mon-ID for Gym-Detection {}', monId)
            ...
            if str(monId) in data:
                ...
                logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) +
                             ') ] ' + 'detectGym: Found other Crops for Mon {}', monId)
                logger.debug(str(crop))
                x1 = crop['X1']
                x2 = crop['X2']
                y1 = crop['Y1']
                y2 = crop['Y2']
        ...
        logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' +
                     'detectGym: Cropsizes: x1:{}, x2:{}, y1:{}, y2:{}', str(x1), str(x2), str(y1), str(y2))
        ...
        if gymHash is None:
            ...
            if self.checkDummy(raidpic, x1, x2, y1, y2, hash, raidNo, radius):
                logger.info('[Crop: ' + str(raidNo) + ' (' +
                            str(self.uniqueHash) + ') ] ' + 'detectGym: Found dummy gym pic')
            ...
            for closegym in closestGymIds:
                ...
                for file in candidates:
                    ...
                    if not foundgym or find_gym > foundgym[0]:
    ...
    def unknownfound(self, raidpic, type, zoom, raidNo, hash, captureTime, imageHash=0, lat=0, lng=0):
        ...
        if imageHash:
            ...
            if not glob.glob(existFile):
                ...
                logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'saveforweb: Write hash file for www: ' +
                             str(type) + "_" + str(lat) + "_" + str(lng) + "_" + str(imageHash) + ".jpg")
            else:
                logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'saveforweb: Hash file for www already exists: ' +
                             str(type) + "_" + str(lat) + "_" + str(lng) + "_" + str(imageHash) + ".jpg")
        else:
            ...
            logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'unknownfound: Write unknown file: ' +
    ...
    def start_detect(self, filenameOfCrop, hash, raidNo, captureTime, captureLat, captureLng, orgFileName, radius):
        ...
        if not os.path.isfile(filenameOfCrop):
            logger.error('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' +
                         'start_detect: File does not exist: {}', str(filenameOfCrop))
            return

        eggfound = False

        logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) +
                     ') ] ' + 'start_detect: Starting analysis of ID {}', str(hash))
        ...
        logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) +
                     ') ] ' + 'start_detect: Got raidtime {}', str(raidtimer))
        ...
        logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' +
                     'start_detect: Determined raidlevel to be {}', str(raidlevel))
        ...
        if raidHash:
            ...
            if lvl == raidlevel and raidlevel is not None:
                ...
                if not mon:
                    ...
                    logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' +
                                 'start_detect: Determined raidlevel to be {}', str(lvl))

                    if lvl is None:
                        logger.error('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' +
                                     'start_detect: Could not determine raidlevel. Filename of Crop: {}', filenameOfCrop)
                        os.remove(filenameOfCrop)
                        os.remove(raidhashPic)
                        return True
                    ...
                    logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' +
                                 'start_detect: Found egg level {} starting at {} and ending at {}. GymID: {}', lvl, raidstart, raidend, gym)
                    ...
                    if raidend[1]:
                        ...
                        logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' +
                                     'start_detect: Submitting mon. ID: {}, gymId: {}', str(mon), str(gym))
        ...
        if not eggfound:
            ...
            if not monFound[0]:
                ...
                logger.warning('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' +
    ...
    def imageHashExists(self, image, zoom, type, raidNo, x1=0.30, x2=0.62, y1=0.62, y2=1.23, radius=0, hashSize=8):
        ...
        logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) +
                     ') ] ' + 'imageHashExists: Hash found: {}', existHash[1])
        return existHash[1]
    ...
    def checkHourMin(self, hour_min):
        ...
        hour_min[1] = hour_min[1].replace(
            'O', '0').replace('o', '0').replace('A', '4')
        if (hour_min[0]).isnumeric() is True and (hour_min[1]).isnumeric() is True:
