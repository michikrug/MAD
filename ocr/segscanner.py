...
class Scanner:
        ...
        if gymHash is None:
            ...
            for closegym in closestGymIds:
                candidates = glob.glob(
                    "ocr/gym_img/_" + str(closegym) + "_.jpg")
    ...
    def successfound(self, raidpic, type, gymId, raidNo, lvl, captureTime, mon=0):
        ...
        curTime = time.time()
        saveFileName = str(type) + "_" + str(curTime) + "__LVL_" + str(lvl) + "__MON_" + str(mon) + "__LAT_" + \
            str(latitude) + "__LNG_" + str(longitude) + "__" + \
            str(gymname) + "__" + str(gymId) + ".jpg"
        log.info('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) +
                 ') ] ' + 'successfound: Filename: ' + str(saveFileName))

        copyfile(raidpic, os.path.join(
            self.args.successsave_path, str(saveFileName)))
    ...
    def start_detect(self, filenameOfCrop, hash, raidNo, captureTime, captureLat, captureLng, orgFileName, radius):
        ...
        if eggfound:
            ...
            if submitStatus:
                self.successfound(filenameOfCrop, 'EGG', gymId,
                                  raidNo, raidlevel, captureTime)
            ...
            if submitStatus:
                self.successfound(filenameOfCrop, 'MON', gymId,
                                  raidNo, raidlevel, captureTime, str(monFound[0]))
