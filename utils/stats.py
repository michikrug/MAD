...
class PlayerStats(object):
    ...
    def stats_collector(self, prototyp):
        ...
        if not self._stats_collector_start:
            if self._last_processed_timestamp != period:
                ...
                data_send_stats.append(self.stats_complete_parser(
                    collect_data, self._last_processed_timestamp))
                data_send_location.append(self.stats_location_parser(
                    collect_data, self._last_processed_timestamp))
                data_send_location_raw = self.stats_location_raw_parser(
                    collect_data, self._last_processed_timestamp)
                data_send_detection_raw = self.stats_detection_raw_parser(
                    collect_data, self._last_processed_timestamp)
    ...
    def stats_complete_parser(self, data, period):
        ...
        logger.debug('Submit complete stats for {} - Period: {}: {}',
                     str(self._id), str(period), str(stats_data))
    ...
    def stats_location_parser(self, data, period):
        ...
        logger.debug('Submit location stats for {} - Period: {}: {}',
                     str(self._id), str(period), str(location_data))
