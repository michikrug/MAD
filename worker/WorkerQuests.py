    ...
    def clear_box(self, delayadd):
        ...
        while int(delrounds) <= 10 and not stop_inventory_clear.is_set():
            ...
            if not first_round and not delete_allowed:
                ...
                logger.warning(
                    'Find no item to delete - scrolling ({} times)', str(error_counter))
                self._communicator.touchandhold(
                    int(200), int(600), int(200), int(100))
                time.sleep(2)
            ...
            while int(trash) <= len(trashcancheck) - 1 and not stop_screen_clear.is_set():
                ...
                check_y_text_ending = int(
                    trashcancheck[trash].y) + self._resocalc.get_inventory_text_diff(self)
                ...
                try:
                    ...
                    if match_one_item:
                        ...
                        delx, dely = self._resocalc.get_confirm_delete_item_coords(self)[0], \
                            self._resocalc.get_confirm_delete_item_coords(self)[
                            1]
                        curTime = time.time()
    ...
    def _handle_stop(self, timestamp: float):
        ...
        while data_received != FortSearchResultTypes.QUEST and int(to) < 4:
            ...
            if data_received == FortSearchResultTypes.INVENTORY:
                ...
                logger.info(
                    'Received new Quest or have previously spun the stop')
                ...
                logger.info(
                    "Likely already spun this stop or brief softban, trying again")
